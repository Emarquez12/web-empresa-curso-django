...
class ThreadTestCase(TestCase):
    ...
    def test_add_message_to_thread(self):
        ...
        message2 = Message.objects.create(user=self.user2, content = "Buenas Noches")
    ...
    def test_find_thread_with_custom_manager(self):
        self.thread.users.add(self.user1, self.user2)
        thread = Thread.objects.find(self.user1, self.user2) # type: ignore
        self.assertEqual(self.thread, thread)
        
    
    def test_find_or_create_thread_with_custom_manager(self):
        self.thread.users.add(self.user1, self.user2)
        thread = Thread.objects.find_or_create(self.user1, self.user2) # type: ignore
        self.assertEqual(self.thread, thread)
        thread = Thread.objects.find_or_create(self.user1, self.user3) # type: ignore
